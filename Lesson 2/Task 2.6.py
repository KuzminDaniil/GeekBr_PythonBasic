...
while True:
    ...
    if int(a) == 3:
        ...
        measure = []
        for a in my_list:
            name.append(my_list[i].get('название'))
            price.append(my_list[i].get('цена'))
            volume.append(my_list[i].get('количество'))
            measure.append(my_list[i].get('eд'))
